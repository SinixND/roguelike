...
namespace snd
{
    ...
    public:
        ...
        void erase(Key const& key) override
        {
            ...
            // Replace removed value with last value before popping (if more
            // than one value exists) to keep values contiguous
            if (values_.size() > 1) {
                ...
                // Replace (removed) value with kept value (by index) so last
                // entry (duplicate) can be popped (becomes new removed)
        ...
        bool contains(Key const& key) override { return keys_.contains(key); };
        ...
        std::vector<Type>& values() { return values_; };
    ...
    private:
        ...
        std::unordered_map<Key, Index>
          keyToIndex_{}; // Key is used to identify value
        std::unordered_map<Index, Key>
          indexToKey_{}; // Store a index (value) to key mapping (internal use

...
#include "/home/arch/coding/git_repos/roguelike/src/Globals/RuntimeDatabase.h"
#include "Constants.h"
#include "Tile.h"
#include "TileMap.h"
...
namespace RayCast
{
      ...
      TileMap& tileMap)
    {
        ...
        for (auto const& rayTarget : rayTargets) {
            ...
            To get the intersections with tile diagonals (which form a relative
            grid coordinate system (aka. csys)) we need to transform the
            direction vector.

              Because we can only easily calculate within our reference (base)
            csys (aka. "frame of reference"), we transform FROM THE RELATIVE TO
            the REFERENCE csys.

            That requires a ROTATION by 45 deg (CW or CCW, does not
            matter), and (see next step) a SCALING FACTOR of SQRT(2)
            (reference unit (TILE_SIZE) is the hypothenuse in the relative
            normalized unit csys).

              To get the ray increment (-> hypothenuse, for x = 1 reference
            unit and y = 1 reference unit) back to the relative system, we need
            to NORMALIZE (both for the "x" and "y" component) the
            hypothenuse / length to the relative systems unit, which is our
            reference unit DOWNSCALED.
            */
            ...
            while (true) {
                ...
                if (minLength > (maxLength + 1)) break;
                ...
                rayEnd =
                  Vector2Add(rayStart, Vector2Scale(rayDirection, minLength));

                // Make vision symmetrical ()
                auto chkx{static_cast<int>(round((rayEnd.x + 12.5) * 10)) % 250};
                auto chky{static_cast<int>(round((rayEnd.y + 12.5) * 10)) % 250};

...
namespace
{
    void processKeyControl(Transformation& cursorPosition)
    {
        ...
        case KEY_UP: {
            if (
              TileTransformation::positionToScreen(
        ...
        if (IsKeyDown(KEY_LEFT_SHIFT)) scale = 4;

        // Check if cursor would go out of screen
        Vector2i newCursorPosition{
          cursorPosition.tilePosition().x + (dir.x * scale),
        ...
        if (
          ...
          !CheckCollisionPointRec(
            TileTransformation::positionToScreen(newCursorPosition),
            GetRenderRec())) {

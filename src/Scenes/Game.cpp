...
void processVision(Unit& unit, TileMap& tileMap)
{
    ...
    while (static_cast<int>(rayTargets.size()) < (targetCount))
    {
        ...
        for (const Vector2i& position : filterInRange(tilesInVisionRange, visionRange, visionRange, unit.position.tilePosition()))
        {
    ...
    for (auto& rayTarget : rayTargets)
    {
        ...
        */
        ...
        while (!tileFound)
        {
            ...
            Tile* tileHit{tileMap.at(worldToTilePosition(rayEnd))};
